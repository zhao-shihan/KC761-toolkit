...
from kc761tool.calib.model import N_CORE, CalibrationModel
from kc761tool.calib.plot import plot_fit
from kc761tool.calib.product import build_calib_product, channel_derived_edges_kev
from kc761tool.calib.report import render_report
from kc761tool.calib.scaling import N_SCALE
from kc761tool.calib.types import (
    DatasetSpec,
    FitProgress,
    FitResult,
    FitSettings,
...
from kc761tool.core.covariance import verify_covariance_psd
...
def run_fit(
    ...
    *,
    ...
    command: str = "kc761tool calib",
    arguments: Sequence[tuple[str, str]] = (),
    producer: str = "kc761tool-calib",
    force: bool = False,
    strict: bool = False,
    ...
    if progress is not None and (not np.isfinite(progress_every_s) or progress_every_s < 0.0):
        raise ValidationError(f"progress_every_s must be finite and >= 0, got {progress_every_s!r}")
