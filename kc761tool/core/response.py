...
from kc761tool.core._gen.kernel_expr import (
    scalar_normalize_derivative,
    scalar_tapered_bin_grad,
)
from kc761tool.core._linalg import csr_from_column_triples
from kc761tool.core.binning import ChannelGrid, EnergyGrid
from kc761tool.core.kernel import (
    N_SIGMA,
    response_triples,
    support_bounds,
)
...
def compose_response(
    ...
    *,
    ...
    if counts.shape[0] != n_deposition:
        raise ValidationError(
            f"deposition_counts has {counts.shape[0]} deposition bins, response has {n_deposition}"
        )
    ...
    for column in prange(centers.size):
        ...
        for row in range(start, stop):
            ...
            for parameter in range(4):
                sums[parameter] += (
                    lo * edge_gradient[parameter, row] + hi * edge_gradient[parameter, row + 1]
                )
...
def response_parameter_jacobian(
    ...
    *,
    ...
    return [
        csr_from_column_triples(rows, cols, normalized[parameter], shape) for parameter in range(7)
    ]
