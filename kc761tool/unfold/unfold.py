...
from kc761tool.unfold.plot import plot_unfold
from kc761tool.unfold.report import render_report
from kc761tool.unfold.selection import select_window
from kc761tool.unfold.solve import SolveOutcome, solve_window
from kc761tool.unfold.types import (
    ComposeResult,
    UnfoldResult,
    UnfoldSettings,
)
...
def run_unfold(
    ...
    *,
    ...
    force: bool = False,
    strict: bool = False,
    command: str = "kc761tool unfold",
    arguments: Sequence[tuple[str, str]] = (),
    producer: str = "kc761tool-unfold",
    plot: bool = True,
    ...
    unfolded = Histogram1D(axis=axis, values=unfolded_values, variances=total_values**2)
    sigma_statistical = Histogram1D(axis=axis, values=stat_values, variances=stat_values**2)
    sigma_systematic = Histogram1D(axis=axis, values=syst_values, variances=syst_values**2)
    sigma_total = Histogram1D(axis=axis, values=total_values, variances=total_values**2)
