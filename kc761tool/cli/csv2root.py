"""``kc761tool csv2root``: strict CSV to spectrum product conversion.
...
def parse_kc761_csv(
    ...
    while index < len(lines) and not lines[index].strip():
        index += 1
    if index >= len(lines):
        raise Kc761toolError(f"{source}: empty CSV file")
    ...
    if not counts:
        raise Kc761toolError(f"{source}: no data rows parsed")
...
def _run(args: argparse.Namespace, *, strict: bool) -> int:
    ...
    if not input_path.is_file():
        raise Kc761toolError(f"input CSV not found: {input_path}")
    try:
    ...
    product = SpectrumProduct(
        ...
        provenance=build_provenance(
            producer="kc761tool-csv2root",
            command="kc761tool csv2root",
