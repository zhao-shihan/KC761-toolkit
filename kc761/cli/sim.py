...
run, ``sys.executable -m kc761 sim ...``, because a ``G4RunManager`` can be
...
def _run(args: argparse.Namespace, *, strict: bool) -> int:
    ...
    if args.events is None:
        ...
        if args.dry_run:
            print(f"kc761 sim (dry-run): interactive {args.source_key} session")
            return 0
...
def _run_interactive(args: argparse.Namespace) -> None:
    ...
    from kc761.sim.runner import macro_path, prepare_interactive
...
def _run_batch(args: argparse.Namespace, *, strict: bool, logger) -> int:
    ...
    for index, run in enumerate(config.runs):
        ...
        if dry_run:
            print(f"kc761 sim (dry-run) [run {index}]: {shlex.join(command)}")
            continue
...
def _expand_run_argv(
    ...
    *,
    ...
    command = [sys.executable, "-m", "kc761", "sim"]
    if run.source_key is not None:
