...
* F-MODEL-1/F-MODEL-2: the fit works in the internal slope basis
  ...
  implementation, in ``kc761/core/_gen/model_expr.py`` (sympy-generated).
...
def verify_resolution_positivity(
    ...
    *,
    ...
    Strict mode raises :class:`kc761.errors.CertificateError` and reports the
...
def verify_energy_monotonicity(
    ...
    *,
    ...
    Strict mode raises :class:`kc761.errors.CertificateError` when the slope
    is not positive.
    """
    maximum = _check_channel_max(channel_max)
    if not strict:
