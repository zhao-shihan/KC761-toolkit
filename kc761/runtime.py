...
``--strict`` or ``KC761_STRICT=1`` (the CLI flag wins). Basic schema, shape and
...
``[kc761.<command>] level: message`` (D-67).
"""
...
class _PrefixFormatter(logging.Formatter):
    """Render ``[kc761.<command>] level: message`` on one line."""
...
def strict_enabled(cli_flag: bool = False) -> bool:
    """Resolve strict mode from the CLI flag and ``KC761_STRICT``.
...
def configure_logging(command: str, level: str = "info") -> logging.Logger:
    """Return the ``kc761.<command>`` logger with one stderr handler.

    Idempotent: repeated calls for the same command reuse the handler.
    """
    logger = logging.getLogger(f"kc761.{command}")
