...
from kc761tool.core.uncertainty import DEFAULT_SYST_FRAC as _DEFAULT_SYST_FRAC
from kc761tool.errors import UsageError
from kc761tool.sim.config import DEFAULT_SEED as _DEFAULT_SEED

#: Registry defaults, imported rather than repeated (D-190).
DEFAULT_SEED = _DEFAULT_SEED
DEFAULT_SYST_FRAC = _DEFAULT_SYST_FRAC
...
def _sim(path: Path, **kwargs):
    """Load a ``[sim]`` table; keys, types and defaults come from SIM_POLICY (D-190)."""
    assert not kwargs
...
def test_unfold_calib_only_rejects_alpha(tmp_path: Path) -> None:
    ...
    with pytest.raises(UsageError, match="calib_only does not use"):
        load_unfold_config(path)
