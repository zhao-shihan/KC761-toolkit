...
def load_spectrum(source: str | os.PathLike | uproot.ReadOnlyDirectory,
    ...
    a path-opened file is closed on exit.  When the file stores no
    ``sumw2`` buffer, uproot falls back to the Poisson ``sqrt(counts)``
    estimate, which is NaN where background-subtracted counts are negative
    -- :class:`Spectrum` rejects such files with an explanation.
    """
