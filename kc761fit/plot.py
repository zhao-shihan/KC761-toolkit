...
def _calibration_panel(ax, c, x_anchors, x_max: float = 2048.0,
                       title: str = "Energy calibration"):
    """Fitted E(x) vs channel with the fitted line positions."""
    x = np.linspace(0.0, x_max, 400)
    ax.plot(x, poly3(c, x), "-", color="tab:purple", lw=1.5,
            label="$E(x) = c_3 x^3 + c_2 x^2 + c_1 x + c_0$")
    ax.plot(x_anchors, CAL_ENERGIES, "o", ms=5, mfc="none",
            color="tab:purple",
...
def _resolution_panel(ax, a, r_anchors, e_max: float,
                      title: str = "Energy resolution"):
    """Fitted sigma(E) with the fitted sigma points.

    The curve is shown from 0 up to ``e_max`` (the energy of the last
    channel under the fitted calibration), so it always covers the full
    measurable range of the data.
    """
    e_res = np.linspace(0.0, e_max, 300)
    sig = sigma_model(a, e_res)
    ax.plot(e_res, sig, "-", color="tab:orange", lw=1.5,
            label=r"$\sigma(E) = a_2 E + a_1 \sqrt{E} + a_0$")
    ax.plot(RES_ENERGIES, RES_ENERGIES * r_anchors, "o", ms=5, mfc="none",
            color="tab:orange",
            label=f"Fit resolution ({'/'.join(f'{e:g}' for e in RES_ENERGIES)} keV)")
    ax.set_xlabel("Energy (keV)")
    ax.set_ylabel(r"$\sigma$ (keV)")
    ax.set_title(title, fontsize=10)
    ax.grid(alpha=0.3)
    ax.legend(fontsize=8, loc="upper left")
...
def plot_global_fit(model, result, out_pdf: str) -> None:
    ...
    _calibration_panel(ax_cal, c, x_anchors, x_max=min_n_bins,
                       title="Energy calibration (global)")
    _resolution_panel(ax_res, a, r_anchors,
                      e_max=float(poly3(c, max_channel)),
                      title="Energy resolution (global)")
    _parameter_panel(ax_params, _parameter_text(result))
