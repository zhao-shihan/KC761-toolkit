...
the sources used to calibrate; in production the `--data` product comes from the
measurement being analyzed.

On the reference campaign the unfold itself takes about 11 s
...
calibration runs. The figures below come from the calibration-source
measurements. The report is wide enough that it is shown at a fixed width; open
the full-size image to read its panel labels.
...
The three figures below are `unfold` results for Th-232, Lu-176 and
...
edges the residuals grow; this is the boundary layer documented in §2.4, so
...
reported bins. Measured on the synthetic fixture (220-520 keV): a truth line in
...
with `T = diag(internal_jacobian(channel_max=ch_max), I_3)`.
...
— above roughly 3.5 MeV on the production 2048-bin axis with the reference
...
F-UNC-1 with
$\Sigma = \mathrm{diag}\big((\mathrm{systFrac}\cdot y)^2\big)$, where
$\mathrm{systFrac}$ is the `syst_frac` input of F-CAL-1.
