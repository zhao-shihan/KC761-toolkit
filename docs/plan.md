...
Status: **final**. The decision register below is the normative specification
for the toolkit; every row is implemented in the code.
Authority: this document is the single specification for the toolkit. When code
and this document disagree, the code is wrong unless a contract point in
Appendix A says otherwise.

## 0. How to read this plan

* Section 1 is the decision register: every frozen decision, grouped by area.
* Sections 2-6 specify the architecture, data contracts, mathematics,
  correctness machinery and engineering rules.
* Section 7 lists risks and required inputs; section 8 defines acceptance.
* Appendix A records the contract points; all of them are resolved (each
  resolution points at the deciding ID). New contract points must still be
...
| D-12 | Product metadata (settings, parameters, provenance) is carried by a single `meta` **RNTuple**. The type is pinned explicitly with `mkrntuple` so the contract does not depend on uproot defaults; the earlier "meta TTree" wording is superseded. |
...
| D-19 | Default outputs live in `work/<subcommand>/` (root revised by D-163; originally `out/`); names encode the inputs and run parameters (e.g. `<calib>-<mode>-n<N>-s<SEED>.root`). **The unfold name carries no `alpha` token (D-192).** |
...
| D-32 | Physics list and cut stay `G4EmPenelopePhysics` with the 0.1 mm default cut; physics behavior does not change. |
| D-33 | Source-mode spectrum and pile-up behavior stay as-is: 4096 bins over 0-4096 keV, 10 us pulse merging. |
...
| D-43 | `R = C . p_tilde . diag(eta)` is composed over the **full** primary axis, then the reported window is sliced. **Padding clause superseded by D-187** (the sliced window is no longer padded). |
| D-44 | The working window is `[chlo - pad, chhi + pad]` for solving and uncertainty propagation; only `[chlo, chhi]` is reported. **Superseded by D-187**: the solve space is the reported window itself and F-BIN-3 is retired. |
| D-45 | Regularization is Tikhonov `chi2 + alpha * ||D mu||**2`; `--alpha` is mandatory (no default, no automatic selection). **The mandatory clause is superseded by D-191**: `alpha` is optional and defaults to 1.0. |
...
| D-48 | Fit weights keep the current formula `var = max(stat, 1) + (syst_frac * data)**2 + MC` with `syst_frac = 0.10`; the formula and the floor are documented. **Default revised to 0.05 by D-169.** |
...
| D-66 | CLI: a single `kc761tool` command with six subcommands `calib`, `unfold`, `sim`, `compose`, `csv2root`, `subbkg`; window options use long names with short aliases; `app/*.py` is deleted. **Revised by D-185**: seven subcommands - `subbkg` is renamed `specsub` and `specadd` is added; both take two positional spectrum operands. |
...
| D-81 | Binning repairs: `working_window` takes `calibration`, `resol_params`, `channel_max` (keyword-only) and converts the local resolution width to channels (**`working_window` is deleted by D-187**); `build_response_matrix` takes the required keyword-only `channel_max` of the internal basis. |
...
| D-83 | Derivative interfaces: F-KERN-4 exposes the **unnormalized** local kernel derivatives (edge, sigma, center, source); the renormalization quotient rule (including the global per-column sum) is assembled once in F-RESP-4 and reused by F-UNC-2. `ProjectionPlan` gains the `weights` matrix. |
...
| D-86 | F-UNC-1/F-UNC-2 use the **reduced** free-set system at a boundary solution (`H_FF^-1` with active variables fixed at zero), not the full inverse Hessian; using the full inverse would overstate the uncertainty of every free direction whenever a constraint is active. Verified against finite differences on 200 random problems. |

### 1.8 Schema and metadata contracts
...
| D-89 | The dedicated `calib_sha256`/`sim_sha256` meta fields are removed; every input fingerprint lives in `provenance.inputs` (path + sha256). The schema layer exposes `fingerprint_for` / `input_sha256` helpers so later stages can verify an input path against a recorded digest. |
| D-90 | `write_product`, `read_product` and `verify_product` take a keyword-only `strict: bool = False`. |
| D-91 | All product-level strict certificates are implemented in the schema layer and run under `strict`; physical-layer certificates remain owned by the calibration, unfolding and simulation layers. |
...
| D-94 | `README.md` belongs to the CLI/documentation layer and is not part of the schema contract. |

### 1.8.1 Metadata finalization
...
| D-97 | `unfold` in `calib_only` mode carries only the common fields plus `mode`; the full mode additionally carries the unfold settings (22 fields since the SNIP mask and D-187/D-188; the original count was 11). |
...
| D-105 | The calibration covariance is the `(c, b)` 7x7 core block of the full-parameter Fisher inverse with the per-dataset scale **marginalized**, scaled by one global `s**2 = chi2/dof` (F-COV-2) and transformed from the internal to the reported basis with the F-MODEL-2 Jacobian (`b` unchanged). Because the Bezier scale has a gauge direction when the scale is (nearly) polynomial, the block is computed with the equivalent, numerically stable Schur complement `(F_cc - F_cs F_ss^+ F_sc)^-1` under Jacobi preconditioning; for an invertible scale block this equals the full-inverse core block. A non-positive-definite core Schur complement is a hard failure: the fit parameters themselves are not identifiable (no pseudo-inverse fallback for the core). |
...
### 1.10 Unfolding contracts

| ID | Decision |
|----|----------|
| D-110 | Exact-zero primary-column pruning (F-UNF-3): after composing over the full primary axis and slicing the reported channel rows (the padded rows before D-187), primary columns whose sliced response is **exactly** zero are removed (no tolerance) and the non-negative solve runs on the remaining columns only. Pruned columns are fixed at zero in the reported solution. |
| D-111 | Energy-to-channel window mapping (F-UNF-1): `E(ch)` is evaluated at the channel **centers**; `chlo`/`chhi` are the first/last channel center inside `[elo, ehi]`, clipped to the acquisition, and at least one center must fall inside. `elo`/`ehi` must lie inside the primary axis range. The reported `mu` covers the primary bins whose centers lie in `[elo, ehi]`. Violations are `ValidationError`. **Revised by D-146** (window entirely outside the acquisition raises instead of degenerating to the top channel). |
...
| D-119 | F-UNC-2 core repair (correction): `simulation_mc_variance` used a rank-one half-gradient derivative `d_js e_s^T` that dropped the `(R^T W C_j) mu_s` vector term and only broadcast for square `n_primary == n_deposition`. The exact multinomial propagation (full-vector derivative, non-centered `sum_j p X^2 - Xbar^2` form so the unrecorded zero-deposition category is included, reduced free-set inverse) replaces it; the F-UNC-2 derivation is corrected and an FD regression is added. |

### 1.11 Simulation contracts
...
### 1.12 Configuration-file mode

| ID | Decision |
|----|----------|
| D-129 | Configuration files are TOML read with the standard library `tomllib` (no new dependency). One file may contain the top-level tables `[sim]`, `[calib]`, `[compose]` and `[unfold]`; each subcommand reads only its own table, and a missing table is a usage error. `csv2root`, `specadd` and `specsub` take no config file (`specadd`/`specsub` per D-185). |
| D-130 | Invocation is `-c/--config FILE` on `sim`, `calib`, `compose` and `unfold`. Config mode is mutually exclusive with the run-selection arguments; only the global options (`--strict`, `--log-level`, `--dry-run`, `--force`, plus the calibration progress controls) are accepted alongside it. Without `--config` the existing flag surface is unchanged. **The enforcement mechanism is replaced by D-190**: the check is the structural scope of each declared option, not a comparison against a mirror table of defaults. |
...
| D-142 | The plot CLI surface (Appendix A item 4) is resolved as a single `--no-plot` switch (plots on by default) for `calib` and `unfold`, with `no_plot` as the config key; `compose` and `sim` produce no plots and redirecting plots to another directory is not supported. **Extended by D-193**: `unfold` also carries the opt-in `--log-plot`/`log_plot` for its logarithmic-y panel. |
...
| D-156 | Peak detection: `y+ = max(y, 0)`; residual from the F-SOLVE-4 baseline; resolution-matched Gaussian significance with threshold `k = 5` standard deviations; bins within `protect_sigma = 2` resolution widths of a candidate are protected; mask floor `0.1`. All values are settings with these defaults. **Protection width superseded by D-188**: the protected half-width is `protect_bins` primary bins (default 3), not resolution widths. **Floor re-tuned by D-191**: the default is `0.01`. |
...
| D-158 | The masked operator is `D_tilde' = diag(rho**0.5) D . diag(sqrt(diag(A)))` with `rho_r = prod_{j=0}^{order} w_{r+j}` (F-SOLVE-6); **the product is revised to `rho_r = min_j w_{r+j}` by D-188, so a touched row carries exactly the floor**; `D_tilde'^T D_tilde'` stays symmetric PSD and banded, and the F-SOLVE-3 KKT certificate is unchanged. A new strict certificate F-SOLVE-6 verifies the mask (**revised by D-188**: the implemented certificate recomputes the weights from the recorded spectrum, settings and iteration reference and enforces `w_i in [0, 1]`; the symmetry and half-bandwidth of `D_tilde'^T D_tilde'` hold by construction and are not re-checked at runtime, and the baseline/mask sha256 in the meta are provenance records -- the mask itself is not stored, so there is nothing to compare against on read). `alpha` remains mandatory and unchanged in meaning. **The mandatory clause is superseded by D-191**: `alpha` is optional and defaults to 1.0, and its meaning is unchanged. |
| D-159 | The data-derived mask makes the reported covariance conditional on the realized mask (plug-in); the mask-selection uncertainty is not propagated. The approximation is documented and coverage is validated on synthetic pulls with the mask on and off. |
| D-160 | `UnfoldSettings`/product meta record `snip_enabled` (default true), `snip_threshold_sigma`, `snip_protect_sigma` (**renamed `snip_protect_bins` and extended with `snip_candidates`/`snip_protected_bins` by D-188**), `snip_floor`, `snip_iterations`, `snip_max_iterations`, the clipped-negative count and index range, and the baseline/mask sha256, so a masked solve is exactly reproducible. |
| D-161 | Implementation is part of the unfolding core (F-SOLVE-4..6 in `core/solver.py`, settings/meta and certificates). Acceptance is quantitative on synthetic spectra: spurious-peak suppression, true-peak area bias, pull coverage with mask on/off, threshold/iteration robustness and bitwise reproducibility; no golden or reference outputs. |
| D-162 | Provisional defaults after the sensitivity study: `snip_enabled = true`, `snip_threshold_sigma = 5`, `snip_protect_sigma = 2` (**replaced by `snip_protect_bins = 3` in D-188**), `snip_floor = 0.1`, resolution-derived `m` capped at `m_max = 8`. The truth-based synthetic closure study (D-161) remains the acceptance gate and may revise them; the realized values are recorded per product (D-160), so a re-tuned default does not invalidate existing products. **Re-tuned by D-191**: `snip_floor = 0.01` and the resolution-derived `m` cap `m_max = 32`. |

### 1.15 Workspace layout and config path resolution
...
| D-165 | `csv2root` and `subbkg` default their output next to the input file: `csv2root` writes `<csv-dir>/<stem>.root`, `subbkg` writes `<signal-dir>/<signal-stem>-subbkg.root`. **Revised by D-185**: the two-operand commands write next to their first operand as `<first-dir>/<first-stem>-add-<second-stem>.root` (`specadd`) and `<first-dir>/<first-stem>-sub-<second-stem>.root` (`specsub`, the renamed `subbkg`). |
...
| D-170 | ROOT titles are display-only and human-readable: the axis canonical name travels in `fName` and its unit is derived from that name via `schema.axes.AXIS_UNITS`; `fTitle` is `Label (unit)` with no parentheses for unitless axes (`channel`/`counts`/`dimensionless`), and is never parsed. Histogram object titles are set from `schema.products.HUMAN_TITLES`. The naming table is fixed in `schema.axes.HUMAN_AXIS_LABELS` and `schema.products.HUMAN_TITLES`. |
| D-171 | All product-writing commands validate their output targets before doing work (extends D-168): `unfold`, `compose`, `sim` (single-run), `calib`, `csv2root` and `subbkg` check the product target and, where a figure is produced by default, the figure target, through `schema.io.validate_output_path` (overwrite policy plus parent creation/writability), raising `UsageError` (exit 2). The `sim` config batch keeps its resume logic and does not pre-check skipped runs. **Extended by D-185**: `specadd` and `specsub` (the renamed `subbkg`) pre-validate their target the same way. |
...
| D-184 | The calibration-compatibility gate for `compose` and the full `unfold` is the **axis contract** (`check_deposition_axes`/`check_data_channel_axis`: `C.y == G.x`, `C.x == data`, `N_j` on `G.y`, all bitwise), which stays a hard `ValidationError` in every mode. The recorded-input digest check (`check_recorded_input`, D-114) is demoted: a digest recorded by the upstream simulation product that no longer matches the calibration file warns (`RuntimeWarning`, message prefixed `D-184` and naming both digests) outside strict mode and raises `ProvenanceError` under `--strict`/`KC761TOOL_STRICT=1`; an absent fingerprint for the calibration path stays silent, exactly as in D-114. Rationale: in matrix mode both the deposition and the primary axis are the calibration product's `C.y` edges (D-121), the Geant4 transport reads no fit parameters, and those edges are themselves derived from the fit (`channel_derived_edges_kev`), so a refit that changes the parameters changes the checked axes anyway, while a refit or file copy that leaves the edges identical is numerically usable and must not force a full re-simulation. Provenance recording is unchanged (D-18/D-89): every consumer of the calibration parameters reads them from the file passed to that invocation, never from the simulation product. The same decision fixes generated-code drift: `ruff.toml` carries `extend-exclude = ["kc761tool/core/_gen"]`, because formatting the generated kernels changes their bytes without changing the generator, which makes the committed manifest digests stale and triggers the import-time regeneration of D-77 that reverts the formatting. |
| D-185 | Two-operand spectrum commands, renamed and extended (revises D-66/D-129/D-165/D-171): `kc761tool subbkg` becomes `kc761tool specsub` (`cli/specsub.py`, logger `kc761tool.specsub`, `producer="kc761tool-specsub"`, `command="kc761tool specsub"`) and the new `kc761tool specadd` sums two spectrum products, so the CLI has seven subcommands. Both take exactly two **positional** operands (`specadd A B`, `specsub A B`); the `--signal`/`--sig`/`--background`/`--bkg` options are deleted rather than aliased, and neither command takes a config file or `--dry-run`. `specsub` subtracts the second operand - the background - scaled by `r = t_A / t_B`, which the documentation records as background subtraction without naming the CLI operands. Both write next to the first operand by default, `<a-stem>-add-<b-stem>.root` and `<a-stem>-sub-<b-stem>.root`; the four existing `work/data/2609a/*-subbkg.root` products were renamed onto that pattern and `examples/` plus `tools/benchmarks.py` follow. The audit-only `source_file` of a combined product is the operator string `"<first> + <second>"` or `"<first> - <second>"` in positional order (revises the D-152 value for these products); the two real input paths and their sha256 remain in `provenance.inputs`. Both commands pre-validate their output target in every mode (D-171) before reading an operand. |
| D-186 | The arithmetic of D-185 is a new layer `kc761tool/spectra/` (package entry plus `combine.py`), which registers F-SPEC-1 and F-SPEC-2 in `docs/derivations.md`; the subtraction previously carried no formula ID, so this also closes an AGENTS hard-rule 9 gap. F-SPEC-1: `values = v_a + v_b`, `variances = max(e_a, 1)**2 + max(e_b, 1)**2`, `daq_time_s = t_a + t_b`. F-SPEC-2: `r = t_a / t_b`, `values = v_a - r v_b`, `variances = max(e_a, 1)**2 + r**2 max(e_b, 1)**2`, `daq_time_s = t_a`, with `e = sqrt(fSumw2)`. Shared always-on preconditions: both operands are `spectrum` products, their channel axes match bitwise (unit and edges) and both DAQ times are finite and positive; every violation is a `SchemaError`/`ValidationError` in both modes, with no clamp or NaN substitution. `cli/specadd.py` and `cli/specsub.py` stay thin wrappers over `kc761tool.spectra.run_specadd`/`run_specsub` (the `cli` layer keeps no numerics), the operation tokens `add`/`sub` are single-sourced in `kc761tool.spectra` and reused for the default output connector, and the resolver `cli/_common.resolve_combination_paths` is shared by both commands. |
| D-187 | (revises D-44/D-110/F-BIN-3/F-UNF-1) The unfold solve space **is** the reported window: the fit rows are the reported channel rows `[channel_low, channel_high]` and the fit primary columns are the reported primary bins `[report_low, report_high]`; the F-BIN-3 row padding (`pad_nsigma`, `--pad-nsigma`, the `pad_nsigma` product meta field, `WindowSelection.solve_low/solve_high`, `core.binning.working_window`) is retired. Rationale (measured, `work/` analysis kept out of the repository): with the padded rows in the chi2, the 9-30 keV padding rows of the Ra-226 dataset (where the composed model and the measured data disagree by ~2 orders of magnitude, so the padded rows are *not* a model-validity region) dominated the weighted chi2 -- an unregularized (alpha -> 0) NNLS comparison on one metric gives chi2 = 62752 with the padded rows and the full column range and 62880 with the padded rows restricted to the reported columns, both with zero 242/295/352 keV area, against chi2 = 209 for the D-187 space (reported rows and reported columns) with those lines recovered (1.49e6/2.35e6/3.74e6); the padded space loses them with the mask on and off alike (shipped alpha = 10: 63855 mask-on / 65349 mask-off, against 276 for the D-187 space). With the report-window fit space the same dataset gives chi2 ~ 276 with all Ra-226 lines recovered and a forward-closure window flux ratio of 0.9998 (vs 0.980 padded). Consequence, documented and pinned by a test: an in-window line whose response leaks outside the window loses the rows that constrain the leaked part, and the strong response collinearity of adjacent primary columns near the edge is then resolved by the roughness penalty toward the boundary, so a boundary layer of several reported bins is leakage-limited when the window is only a few resolution widths wide (fixture, 220-520 keV: a truth line in the second-to-last reported primary bin recovers 0.20 of its amplitude vs 0.87 with the padded rows, its strength moves outward into the last reported bin, and a line six reported bins inside loses 15%; widening the window to 200-560 keV restores 0.88-0.91, and the 1336-row production window closes at a total flux ratio of 0.9998); users who need quantitative edge bins must widen the window (the padding convention can be reproduced by reporting a window wider than the region of interest) because the toolkit no longer fits a region the user did not request. `pad_nsigma` removal is a CLI and product-schema break: the unfold `meta` field set changed, so pre-D-187 unfold products are rejected on read (the field-mismatch message names the possibility), while `format_version` stays 1 because calib/sim/spectrum/compose products are byte- and field-identical and a global bump would reject valid inputs. Only *full-mode* unfold products are affected: a pre-D-187 `calib_only` product carries the common fields plus `mode` and still validates. |
| D-188 | (revises D-156/D-158/D-160/D-162) SNIP peak-mask redesign. (a) The protection half-width is `snip_protect_bins` **primary bins** (default `3`, the order-2 stencil width), replacing the resolution-scaled `protect_sigma`; the detector resolution stays in the *detection* step (matched filter of F-SOLVE-5). Rationale: on the production 2048-bin axis `sigma_E/Delta_E` is 5.5-20 bins, so the old rule protected 46.8% of the axis (80% of 130-650 keV, runs up to 96 bins), which is not a local prior; the resolution-scaled and fixed-width rules coincide only when a bin is comparable to the resolution, which this deployment does not satisfy (`sigma_E / Delta_E` >= 5.5 bins, so a cap always binds) and a finely binned deployment satisfies only when `sigma_E <= 1.5` bins, where the peak is no longer resolved; a capped resolution rule would therefore leave `protect_sigma` a silent no-op on one side and a bin-scale protection would be the meaningful one on the other. (b) The masked row weight is `rho_r = min_j w_{r+j}` instead of the stencil product, so a row that touches a protected bin carries exactly the configured `snip_floor`; the product made the realized relaxation `floor**(order+1)` = 1e-3, 100x stronger than the documented "--snip-floor penalty weight" and enough to make the masked QP degenerate (measured on the Ra-226 dataset: a 4-spike comb at 609 keV with a 2.3x single-bin height in the reported-window fit space and 4.8x in the padded space of the pre-D-187 pipeline, 186/242 keV split into two maxima, non-zero bins halved at constant total strength). With (a)+(b) the protected fraction is 6.2%, no peak splits, and the peak-height inflation is 1.23x. (c) The SNIP iteration count uses the reported-window midpoint (D-157) through a new `iteration_reference_index` argument, not the middle bin of the full axis. (d) `snip_baseline` no longer applies the clipping average at the axis ends: a bin with a missing `i +- p` neighbor keeps its value, which is what the documented "edge-preserving, not pulled down" rule always claimed (the former one-sided average lowered a boundary value toward its in-range neighbor). (e) The product meta carries `snip_protect_bins`, `snip_candidates` and `snip_protected_bins` so the realized mask coverage is auditable; the counters and the mask sha256 describe the **full measured axis** (the hash is taken before the reported-window slice), while the penalty sees only the reported-window slice of the mask. |
| D-189 | The unfold figure keeps a **linear** energy x-axis; only the y-axis of the middle panel is logarithmic. This narrows the "log axes" convention of D-153 to the calibration figure and is recorded because D-153 named log axes as a shared figure convention. **The panel set is revised by D-193**: the logarithmic-y panel exists only with `--log-plot`, and the linear x-axis is unchanged. |
| D-190 | (revises the enforcement mechanism of D-130) Every subcommand declares its option surface **once** in `kc761tool/cli/_registry.py` (`RunPolicy`/`RunOption`): flag spellings, type, default, scope, TOML key, requirement and per-value/cross-field checks. `add_run_options` builds the parser from that declaration and gives run-scoped options `default=argparse.SUPPRESS`, so "the user provided this" is an attribute-presence fact; `resolve_run_options` is the only place that applies defaults, checks requirements, rejects retired spellings, detects the config conflict and validates; the TOML loaders derive their key sets, types and defaults from the same declaration and call the same `check_policy`, and the `--config` help text renders the GLOBAL option list from the policy. Consequences: the mirror table `_RUN_ARG_DEFAULTS` and the value comparison `getattr(args, dest) != default` are gone (they made "explicitly passed the default value" undetectable, which silently ignored any `--snip-*` value in config mode because that family was missing from the table, and every default-valued option of `unfold`/`sim` otherwise); a value can no longer be valid on one surface and invalid on the other, because the CLI validation *is* the library validation (`UnfoldSettings`, `SnipSettings`, `FitSettings`); the retired-spelling rejection is a declared entry rather than a hand-written shim; and the CLI surface is unchanged (the seven help screens differ from the pre-D-190 tree only by the D-187/D-188 flag changes and by the `--config` help naming the true global set). Registration is argparse-only, so D-129's "no new dependency" holds. |
| D-191 | **Unfold default parameters** (revises D-45, D-156, D-158 and D-162, using the re-tuning clause that D-162 itself records): a full unfold defaults to `alpha = 1.0`, `snip_floor = 0.01` and `snip_max_iterations = 32`. The three values are single constants in `kc761tool/core/solver.py` (`DEFAULT_ALPHA`, `DEFAULT_SNIP_FLOOR`, `DEFAULT_SNIP_MAX_ITERATIONS`) that the CLI declaration (D-190), the `[unfold]` TOML loader and the library (`run_unfold`, `UnfoldSettings`, `RegularizationSpec`) all read, so no surface can disagree. `--alpha`/`alpha` become optional, which supersedes the "`--alpha` is mandatory" clause of D-45; `alpha = None` remains the internal marker of the `calib_only` path (which solves no QP and still rejects an alpha supplied explicitly on either surface) and passing `None` to a full-mode `run_unfold` is still a `ValidationError`, never a silent substitution. Rationale: 1.0 / 0.01 / 32 are the operator's tuned production values for the 2048-bin deployment (the `work/unfold/` products of 2026-09-13 record exactly `alpha = 1.0`, `snip_floor = 0.01`, `snip_max_iterations = 32`), and the realized values are recorded per product (D-160), so products written with the D-162 defaults stay readable, valid and reproducible from their own `meta`. |
| D-192 | **The default unfold output name drops the alpha token** (revises the D-19 name convention recorded in `docs/formats.md` section 7.2): the default is `unfold-<data-stem>-<sim-stem>.root`, not `unfold-<data-stem>-<sim-stem>-a<alpha>.root`; `unfold --calib-only` keeps `unfold-<data-stem>-calibonly.root`. Rationale: `alpha` now defaults to 1.0 (D-191), so the token adds no information to a default invocation and only lengthens an already long name; a non-default `alpha` is distinguished by the user through `-o/--output`, and an accidental collision is still refused up front by the D-17/D-171 overwrite check. The realized `alpha` stays in the product `meta` (D-160/D-191), so the file name is not the provenance record. |
| D-193 | **The logarithmic-y panel is opt-in in the unfold figure** (revises the three-panel geometry of D-153 and the panel clause of D-189; extends the D-142 plot surface). The default figure is the linear-y spectrum panel with the relative-residual panel below it (height ratio 2:1) in full mode and the linear-y spectrum panel alone in `calib_only` mode; the logarithmic-y spectrum panel that used to sit between them is produced only with `--log-plot` (`log_plot` as the `[unfold]` config key), and `--log-plot` inserts it between the two exactly as before. The energy x-axis stays linear (D-189). Panel heights follow the panel count through `_SPECTRUM_PANEL_IN`/`_RESIDUAL_PANEL_IN` in `kc761tool/unfold/plot.py` (the constants carry both the figure height and the drawn ratio), so `--log-plot` reproduces the pre-D-193 full-mode geometry exactly (10.5 in overall, 2:2:1) while the default figure is shorter instead of leaving an empty slot. The one geometry that changes height is the opt-in `calib_only` pair: D-153's 10.5 in with equal-height panels becomes 8.4 in with two equal panels, because every spectrum panel now carries the same nominal height. |
| D-194 | **Chinese is the default README and English is the technical baseline** (replaces the translation layout this entry first recorded; a deliberate, bounded exception to the English-language rule of D-5/D-181/D-182, which AGENTS.md hard rule 1 restates). The repository front page `README.md` is written in Chinese and the English text sits beside it as `README.en.md`; the two are full section-by-section mirrors of each other - same section and heading structure, same formula IDs (`F-*`), decision IDs (`D-*`), product object names, file paths, CLI flags, config keys and code blocks, all of which stay verbatim in both - and they carry a mutual language link at the top. The Chinese file is soft-wrapped - one physical line per paragraph, list item and table row, with fenced code blocks and display-math blocks left verbatim - because a hard line break inside a Han-character paragraph renders as a space; the English file keeps its wrapped layout. Math in either file keeps underscore-bearing identifiers out of the text-mode commands (`\text{}`, `\texttt{}`): markdown consumes the `\_` escape before the math renderer sees the span, which fails as `'_' allowed only in math mode`, so such an identifier is written as a code span (or replaced by the equivalent math symbol defined elsewhere) instead. **The English `README.en.md` remains the technical baseline**: on any divergence the English text wins, because every other artifact (`docs/*.md`, the code, CLI help) is English, and a change to either README updates the other in the same change. The Chinese terminology follows nuclear-detection and particle/nuclear-physics usage with the English term in parentheses at first occurrence (脉冲幅度谱 (pulse-height histogram), 能量刻度, 能量分辨率, 解谱 (unfolding), 重褶积谱 (refolded spectrum), 响应矩阵, 本底扣除, 正则化, 道址 (channel), 不确定度, 运行时校验 (certificate), 数据产品 (product), 溯源 (provenance), 严格模式). Every other repository artifact - `docs/*.md`, `AGENTS.md`, docstrings, comments, CLI help, error messages and identifiers - stays American English, so hard rule 1 is narrowed by exactly one file (the Chinese `README.md`) plus the glossary quoted in this entry, and is otherwise unchanged. **The READMEs describe the author's own analysis campaign, never repository contents**: measured spectra and data products are not part of the tree (`work/` is untracked user state, D-179), so the text must not present them, or any test case built from them, as shipped with the repository - the only committed analysis artifacts are the figures under `examples/plots/` and the TOML examples under `examples/`. Nothing functional is touched: no formula, constant, axis convention, product key, object name or CLI surface changes, and no gate or CI job reads the READMEs (D-180). |
...
kc761tool/
  ...
  core/                       # pure numerics: stdlib + numpy/scipy/numba only, no IO
    ...
    binning.py                # channel/energy grids (D-79; the pad is retired, D-187)
  ...
  schema/                     # product contracts and IO (uproot)
    ...
    _uproot.py                # verified low-level uproot helpers
  calib/                      # fit orchestration, Bezier scaling, report, plots
  unfold/                     # window selection/orchestration, report, plots
  spectra/                    # F-SPEC-1/F-SPEC-2 combination behind specadd/specsub
...
* `spectra` owns the spectrum arithmetic of `specadd`/`specsub` (F-SPEC-1/F-SPEC-2);
  it imports `core` types where unavoidable and `schema` for products and IO,
  never `cli`, `calib`, `unfold`, `sim` or Geant4.
...
| spectrum | `kc761_spectrum` (TH1D, counts, fSumw2), `meta` | Produced by `csv2root`, `specadd` and `specsub`; carries `daq_time_s`. |
...
* calib adds: `channel_max`, `params_reported_json`, `resol_params_json`, and
  the calibration diagnostics `chi2`, `dof`, `covariance_scale`, `fit_status`,
...
* unfold adds: `mode` (str), `alpha`, `difference_order`, `energy_low_kev`,
  `energy_high_kev`, `channel_low`, `channel_high`, `syst_frac`,
  `chi2`, `dof`, `covariance_scale`, plus the SNIP fields listed in D-160/D-188;
  calib-only carries only `mode`.
...
| F-BIN-3 | **Retired by D-187**: the padded working window `[chlo - pad, chhi + pad]`; the solve space is now the reported window itself (`core/binning.py`, `unfold/selection.py`) |
...
| F-SOLVE-4 | SNIP LLS baseline (transform, resolution-derived iteration count at the reference bin, `max(y,0)` handling, edge-preserving ends) (D-155/D-157/D-188) |
| F-SOLVE-5 | resolution-matched peak significance and the fixed peak mask `W` with a `protect_bins` width (D-156/D-188) |
| F-SOLVE-6 | masked operator `D_tilde' = diag(rho**0.5) D . diag(sqrt(diag(A)))` with `rho_r = min_j w_{r+j}`, mask certificate and the pending acceptance metrics (D-158/D-161/D-188) |
...
| F-UNF-1 | energy window to channel rows and reported primary bins; the solve space is that window (D-187) |
...
* Documentation: `README.md` (Chinese, the default) with its English mirror
  `README.en.md`, `docs/architecture.md`, `docs/formats.md`,
...
2. **CSV sample provided.** The raw files are in `work/data/2609a/`
   (`Channel,Count` header with a `#<...>` acquisition time); the strict
   `csv2root` grammar, ranges and error policy are frozen in
   `docs/formats.md` section 7.3 (D-72; Appendix A item 8).
...
6. the replacement is complete: no superseded implementation remains.

## Appendix A. Contract points (all resolved)

1. **Non-strict resolution negativity.** Resolved by D-73:
...
2. **SNIP peak masking.** Resolved by D-154..D-161 and revised by D-188: a
   redesigned, default-on SNIP peak mask is part of the Tikhonov operator
   (F-SOLVE-4..6), with the protection localized to a fixed number of primary
   bins and the masked row weight equal to the configured floor. D-74 is
   superseded. Full derivation and limits in `docs/derivations.md`.
3. **`calib --sim` naming.** Resolved by D-144: the option is
   `calib --mc` with no alias, and the `[[calib.datasets]]` key is `mc`;
   `compose`/`unfold` keep `--sim`/`sim` for the matrix simulation product.
4. **Plot CLI surface.** Resolved by D-142: a single `--no-plot`
   ...
   supported. Extended by D-193: `unfold` also carries the opt-in
   `--log-plot`/`log_plot` for its logarithmic-y panel.
5. **Optimizer controls.** D-47 fixes a single quasi-Newton stage; the CLI
   flags for iteration limits/tolerances are `--max-iter` / `--tolerance`
   (D-145).
6. **Source key registry.** Resolved by D-122: the seven source keys are
   frozen in the simulation registry with per-entry provenance; additions or
   renames require a new contract decision.
7. **Format version numbering.** Resolved by D-78: the new
   products start at `format_version = 1`, independent of the calibration-file
   version 3.
8. **csv2root strict parser details.** Resolved: the grammar is frozen in
   `docs/formats.md` section 7.3 (header, DAQ time, three-field rows, channel
   contiguity, accepted ranges) and exercised on the provided
   `work/data/2609a/` sample.
