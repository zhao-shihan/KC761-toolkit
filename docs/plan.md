...
| D-162 | Provisional defaults after the sensitivity study: `snip_enabled = true`, `snip_threshold_sigma = 5`, `snip_protect_sigma = 2`, `snip_floor = 0.1`, resolution-derived `m` capped at `m_max = 8`. The truth-based synthetic closure study (D-161) remains the acceptance gate and may revise them; the realized values are recorded per product (D-160), so a re-tuned default does not invalidate existing products. |
...
2. **SNIP peak masking.** Resolved by D-154..D-161: a redesigned,
   default-on SNIP peak mask is part of the
