...
| ID | Decision |
|----|----------|
| D-172 | Performance umbrella; supersedes the "deferred" clause of D-151. The response kernel is assembled with a vectorised support pattern (one `searchsorted` over all columns plus one generated-expression call over the flattened entries); the generated kernels apply `sympy.cse`; the normal equations use dense BLAS above a measured crossover; `core._linalg` exposes a reusable `SpdFactor`; the active-set solver densifies a dense Hessian once and slices with `numpy.ix_`; `compose_parameter_jacobian` accepts the solve window; `simulation_mc_variance` uses the D-173 streaming path. Definitions and certificates are unchanged. |
