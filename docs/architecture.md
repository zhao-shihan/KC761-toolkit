...
1. `kc761tool/core` imports only the standard library plus `numpy`, `scipy` and
...
* `kc761tool/cli/sim.py` config mode expands each `[[sim.runs]]` entry into
  `sys.executable -m kc761tool sim ...` and runs the children serially from the
...
* The fixed source-mode Monte-Carlo axis (`0..4096 keV / 4096 bins`) is single
  ...
  `kc761tool.calib` imports it for the parameter-independent fit-time `C_fit`, and
  `kc761tool.sim` uses it for the source-mode `mc_spectrum`. The matrix-mode primary
...
* `kc761tool/core/_gen/` is generated by `tools/generate_kernels.py` and guarded
...
* **`sim`** is the Geant4 simulation layer (`kc761tool/sim/`, the `sim`
