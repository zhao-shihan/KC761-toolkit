"""Deposition-to-channel matrix rebuild for the systematic-error propagation.

The nominal deposition-to-channel matrix comes from the calibration file (kc761unfold.reader);
the propagation of the calibration-parameter uncertainty needs the
deposition-to-channel matrix at perturbed parameters ``(c0..c3, b0..b2)``.  The rebuild
...
def _rebuild_dense(e_edges, resol_params, ch_lo, n_bins):
    """Dense subrange deposition-to-channel matrix R[i, j]."""
...
def rebuild_deposition_to_channel(calib_coeffs: np.ndarray, resol_params: np.ndarray,
                     n_channels: int, channel_low: int,
                     channel_high: int) -> sparse.csr_matrix:
    """Rebuild the thresholded subrange deposition-to-channel matrix."""
